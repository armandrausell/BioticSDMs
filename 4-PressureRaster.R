...
compute_competitor_pressure <- function(species_name, n = 10, sizeWeight = TRUE) {
  # Step 1: Get top competitors and their pressures
  if(sizeWeight == TRUE) {
  competitors_data <- get_top_competitors(species_name, n, sizeWeight = TRUE)
  } else {
    competitors_data <- get_top_competitors(species_name, n, sizeWeight = FALSE)
  }
...
evaluate_competitor_correlation <- function(species_name, max_competitors = 15, sizeWeight=TRUE) {
  ...
  for (n in 1:max_competitors) {
    ...
    if(sizeWeight == TRUE) {
      pressure_map <- compute_competitor_pressure(species_name, n, sizeWeight = TRUE)
    } else {
      pressure_map <- compute_competitor_pressure(species_name, n, sizeWeight = FALSE)
    }
  ...
  for (species in unique_species) {
    ...
    species_results <- evaluate_competitor_correlation(species, max_competitors = 15, sizeWeight = F)
    all_correlation_results <- rbind(all_correlation_results, species_results)
  }
  
  # Save the computed data to CSV
  write.csv(all_correlation_results, "Unweighted_opt_competition.csv", row.names = FALSE)
...
Opt_competitor_per_species_weighted <- all_correlation_results %>%
  group_by(Species) %>%
...
df_combined <- bind_rows(
  Opt_competitor_per_species %>%
    dplyr::select(Species, Corr_Act_Biserial) %>%
    mutate(Treatment = "Weighted"),
  
  Opt_competitor_per_species_weighted %>%
    dplyr::select(Species, Corr_Act_Biserial) %>%
    mutate(Treatment = "Unweighted")
)

# Order species based on ascending correlation in the Weighted treatment
species_order <- df_combined %>%
  filter(Treatment == "Weighted") %>%
...
ggsave("Lollipop_graph.jpg", plot = l1, width = 11, height = 6, dpi = 600)
